...
<head>
    ...
    <script>var shop = new wiiShop(); const unused = shop.connecting;</script>
    <script type="text/JavaScript">
        <!--
        var testMode = 'false';
    ...
    <script type="text/javascript">
            ...
            function Redirect() {
                ...
                var queryStr = Str.split("&");
                var EncodedParameters = [];

                for (var i = 0; i < queryStr.length; i++) {
                    var keyValuePair = queryStr[i].split("=");
                    var key = encodeURIComponent(keyValuePair[0]);
                    var value = encodeURIComponent(keyValuePair[1]);
                    EncodedParameters.push(key + "=" + value);
                };

                var encodedQueryStr = EncodedParameters.join("&");
                var redirTo = "B_04.jsp";

                window.location.href = redirTo + "?" + encodedQueryStr;
            }
    </script>-->
...
<body style="overflow: hidden;" onload="/*Redirect();*/initPageCommon();">
    <%
